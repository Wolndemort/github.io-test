...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    ...
    if (settings && location.pathname.startsWith("/staff/settings/")) {
      ...
      const fields = {branding: [["club_name","Club name"],["logo_url","Logo URL"],["theme","Theme"]], limits: [["session_timeout_minutes","Session timeout"],["freeze_price_per_day","Freeze price/day"],["max_upload_mb","Max upload MB"],["max_students","Max students"]], features: [["freeze","Freeze enabled"],["qr_checkin","QR check-in enabled"],["online_payments","Online payments enabled"]], integrations: [["email_enabled","Email enabled"],["push_enabled","Push enabled"]], notifications: [["email_enabled","Email enabled"],["push_enabled","Push enabled"],["telegram_enabled","Telegram enabled"]], menu: [["show_schedule","Show schedule"],["show_shop","Show shop"],["show_payments","Show payments"]]}[section];
      if (fields) { const endpoint = section === "integrations" ? "/api/v1/staff/settings/integrations" : section === "notifications" ? "/api/v1/staff/settings/notifications" : "/api/v1/staff/settings/club"; const bools = ["freeze","qr_checkin","online_payments","email_enabled","push_enabled","telegram_enabled","show_schedule","show_shop","show_payments"]; settings.parentElement.appendChild(form(`<h2>Update ${section}</h2><form>${fields.map(([key,label]) => `<label>${label}<input name="${key}" ${bools.includes(key) ? "type=checkbox" : ""}></label>`).join("")}<button>Save settings</button></form><p data-operation-result role="status"></p>`, async event => { event.preventDefault(); const f = new FormData(event.target), data = {}; fields.forEach(([key]) => { data[key] = bools.includes(key) ? f.get(key) === "on" : f.get(key); }); const result = event.currentTarget.querySelector("[data-operation-result]"); try { await post(endpoint, section === "integrations" || section === "notifications" ? data : {[section === "branding" ? "branding" : section]: data}); result.textContent = "Settings saved."; } catch (_) { result.textContent = "Settings unavailable or disabled."; } })); }
    }
