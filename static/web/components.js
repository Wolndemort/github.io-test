...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    ...
    if (location.pathname === "/staff/sales" && sales) sales.appendChild(form('<h2>Cash subscription sale</h2><form><input name="student_id" type="number" min="1" placeholder="Student ID" required><input name="discipline" placeholder="Discipline" required><input name="tariff_idx" type="number" min="0" placeholder="Tariff index" required><button>Activate</button></form><p data-operation-result role="status"></p>', async event => { event.preventDefault(); const f = new FormData(event.target), result = event.currentTarget.querySelector("[data-operation-result]"); try { const data = await post("/api/v1/staff/sales/cash-subscription", {student_id: Number(f.get("student_id")), discipline: f.get("discipline"), tariff_idx: Number(f.get("tariff_idx")), discount_ids: []}); result.textContent = `Subscription ${data.order_id} saved.`; } catch (_) { result.textContent = "Subscription unavailable or disabled."; } }));
