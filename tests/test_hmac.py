...
def test_signature_consistancy():
...
def test_signature_changes_on_input_change():
    ...
    user_id = 1
    salt = 'salt'
    origin_signature = (user_id , salt)

    assert origin_signature != generate_signature(2, salt)
    assert origin_signature != generate_signature(1, 'salt2')


@pytest.mark.parametrize("user_id, time_salt",[
    (0, ""),   # Крайний случай: ID ноль и пустая соль
    (999999999, 'abc'),  #Большой ID
    (-1, '!!!')  #отрицательный id
])
