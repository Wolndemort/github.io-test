...
Уточнение по сравнению с Telegram WebApp: legacy `/admin` — это отдельная полноценная страница `📋 Таблица WebApp` (`templates/admin.html`), а `/stats` — полноценная статистика (`templates/stats.html`). Legacy `/forecast` (`templates/forecast.html`) содержит фильтры, список прогнозируемых атлетов, суммы продлений, тарифы/дисциплины и два Chart.js графика (выручка и посещения). Текущий Web `/staff/overview` показывает только 3 KPI-карточки, `/staff/forecast` только 3 KPI-карточки, `/staff/students` только короткий список без карточек и действий. Поэтому пользователь прав: это не проблема одной ссылки — Web-миграция пока перенесла только урезанные proof-of-concept представления и не перенесла содержимое «Таблица», «Статистика» и полноценного «Прогноза».
