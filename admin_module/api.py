...
async def get_qr_scanner(request: Request, club_id: int = Query(...)):
    # Telegram can load the WebApp shell before exposing initData in the URL.
    # scanner.html reads it from Telegram.WebApp and the scan endpoint performs
    # the actual authentication, so the page itself must not return 401 here.
    ...
    if payload.expire_date is not None:
        if payload.expire_date == "":
            ...
            try:
                ...
                if not expire_date:
                    raise ValueError
                student.expire_date = datetime.combine(expire_date, datetime.max.time().replace(microsecond=0))
            except ValueError:
    ...
    try:
        ...
        receipt = build_owner_receipt_text(
                ...
                items_text=format_order_items([
                    type("ItemView", (), {"title": f"{str(cfg.get('name', discipline))} · {tariff_label}", "quantity": 1, "product_id": 1})()
                ]),
                amount_kopecks=price,
                extra_lines=[
                    f"Атлет: <b>{escape(student.name or str(student.id))}</b>",
...
async def get_revenue_stats(
    ...
    return templates.TemplateResponse(
        "stats.html",
        {
            ...
            "deploy_expenses_today": cash_flow.get("today_deploy_expense", 0),
            "expenses_week": cash_flow["week_expense"],
            "deploy_expenses_week": cash_flow.get("week_deploy_expense", 0),
            "expenses_month": cash_flow["month_expense"],
            "deploy_expenses_month": cash_flow.get("month_deploy_expense", 0),
